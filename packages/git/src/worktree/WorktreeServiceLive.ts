...
import {
  ...
  type BranchCollisionResolution,
  type BranchPublication,
  type CleanupWorktreeInput,
  type CommitWorktreeInput,
  type CreateDisposableWorktreeInput,
  type CreateImplementationWorktreeInput,
  type CreateOrUpdateBranchInput,
  type InspectWorktreeInput,
  type ResolveBranchCollisionInput,
  type RetainWorktreeInput,
  type WorktreeCommit,
  type WorktreeDiff,
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const ensureRepository = (repositoryPath: string) =>
      Effect.gen(function* () {
        const cwd = normalizePath(repositoryPath);
        const topLevel = yield* gitString(cwd, ["rev-parse", "--show-toplevel"]);
        const gitDir = yield* gitString(cwd, ["rev-parse", "--absolute-git-dir"]);

        return {
          gitDir: normalizeExistingPath(gitDir),
