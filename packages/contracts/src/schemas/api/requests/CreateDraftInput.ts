...
export const CreateDraftInput = Schema.Struct({
  ...
  body: Schema.optional(Schema.String).pipe(
    Schema.annotateKey({ description: "Optional markdown body for the draft." }),
  ),
  ...
  externalLinks: Schema.optional(Schema.Array(ExternalLinkInput)).pipe(
    Schema.annotateKey({ description: "Optional external links to attach to the draft." }),
  ),
  ...
  planningNotRequired: Schema.optional(Schema.Boolean).pipe(
    Schema.annotateKey({ description: "Whether the draft can skip planning acceptance checks." }),
  ),
  ...
  source: Schema.optional(Schema.Unknown).pipe(
    Schema.annotateKey({
      description: "Importer-owned source data preserved as an explicit extension field.",
    }),
  ),
  status: Schema.optional(Schema.String).pipe(
    Schema.annotateKey({ description: "Optional initial workflow status." }),
  ),
  title: Schema.String.pipe(Schema.annotateKey({ description: "Draft title." })),
  ...
  Schema.annotate({
    description: "Payload for creating an editable issue draft.",
    identifier: "@cycle/contracts/CreateDraftInput",
    title: "CreateDraftInput",
  }),
);
