...
import type { AgentActiveTurnDirectoryShape } from "./runtime/ActiveTurnDirectory.ts";
import { messageFromTurnResult } from "./prompt.ts";
...
export async function* chatTurnSseFrames(input: {
  ...
  try {
    ...
    input.activeTurns.finish(input.provider, input.sessionId, terminalStatus ?? "cancelled");
