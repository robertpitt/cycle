      ...
      return decodeHttpValue(
        ...
        Effect.map((input) =>
          HttpServerResponse.isHttpServerResponse(input)
            ? input
            : useCaseInvocation(IssueSearch, scoped(params.repositoryId, input), meta(requestId)),
        ),
      );
    })();
    const resolvedUseCase = yield* useCase;
    if (HttpServerResponse.isHttpServerResponse(resolvedUseCase)) return resolvedUseCase;
    const pageValue = yield* resolvedUseCase.run;
...
export const createIssue = ({ params, payload, request }: V1Request<"createIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
...
export const getIssue = ({ params }: V1Request<"getIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const result = yield* runUseCase(
      IssueGet,
...
export const updateIssue = ({ params, payload, request }: V1Request<"updateIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
...
export const transitionIssue = ({ params, payload, request }: V1Request<"transitionIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const input = yield* decodeHttpValue(
...
export const archiveIssue = ({ params, payload }: V1Request<"archiveIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const input = yield* decodeHttpValue(
...
export const restoreIssue = ({ params, payload }: V1Request<"restoreIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const input = yield* decodeHttpValue(
...
export const listIssueHistory = ({ params, request }: V1Request<"listIssueHistory">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const url = urlFromRequest(request);
    const input = yield* decodeHttpValue(
...
export const getIssueRevision = ({ params }: V1Request<"getIssueRevision">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const input = yield* decodeHttpValue(
...
export const diffIssue = ({ params, request }: V1Request<"diffIssue">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const url = urlFromRequest(request);
    const fromSnapshotId = url.searchParams.get("fromSnapshotId") ?? url.searchParams.get("from");
...
export const addIssueRelation = ({ params, payload }: V1Request<"addIssueRelation">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const input = yield* decodeHttpValue(
...
export const listIssueRecords = ({ params, request }: V1Request<"listIssueRecords">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
    const url = urlFromRequest(request);
    const input = yield* decodeHttpValue(
...
export const addIssueRecord = ({ params, payload, request }: V1Request<"addIssueRecord">) =>
  Effect.gen(function* () {
    const { requestId } = yield* CycleRequestContext;
