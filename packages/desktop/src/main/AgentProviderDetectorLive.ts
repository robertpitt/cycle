...
import { appConfigError, type AppConfigError } from "../shared/AppConfig.ts";
import {
...
const nvmExecutableDirectories = (
  env: AgentProviderDetectionEnvironment,
): Effect.Effect<ReadonlyArray<string>, AppConfigError> => {
...
const findExecutable = (
  executable: string,
  env: AgentProviderDetectionEnvironment,
): Effect.Effect<string | undefined, AppConfigError> =>
  Effect.gen(function* () {
...
const findShellExecutables = (
  env: AgentProviderDetectionEnvironment,
): Effect.Effect<ReadonlyMap<string, string>, AppConfigError> => {
...
): Effect.Effect<ReadonlyArray<DetectedAgentProvider>, AppConfigError> =>
  Effect.gen(function* () {
